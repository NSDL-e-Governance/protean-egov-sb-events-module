...
export class JoinEventComponent implements OnInit {
	...
	ngOnInit() {
		this.eventConfig = _.get(
			this.libEventService.eventConfig,
			"context.user",
		);
	...
	async joinEvent() {
		...
		var startDifference =
			startEventTime.getTime() - todayDateTime.getTime();
		...
		this.toShowCounter =
			startInMinutes <= 0 && endInMinutes < 0 ? false : true;
	}
	...
	checkEventProvider() {
		if (this.eventDetailItem.onlineProvider == "BigBlueButton") {
					...
					.subscribe(
						(data) => {
							this.openProviderLink(
								data.result.event.attendeeMeetingLink,
							);
						},
						(err: any) => {
							this.sbToastService.showIziToastMsg(
			...
			if (this.eventDetailItem.onlineProviderData["meetingLink"]) {
				...
				this.sbToastService.showIziToastMsg(
					"Join meeting link is not available.",
					"error",
				);
			}
		}
	...
	letModaratorJoinBBB(muted) {
		...
		this.eventService
			...
			.subscribe(
				(data) => {
					this.openProviderLink(
						data.result.event.moderatorMeetingLink,
					);
				},
				(err: any) => {
					this.sbToastService.showIziToastMsg(
